...
char Drives[50];
pthread_t ptimer;
char current_drive[20];
int current_drive_music_count,playing_song_no;
        ...
        if(ch == -32)
            {
                ...
                if(ch == up || ch == down)
                {
                    ...
                    if(ch == up)
                    {
                        if(b1>6 && b1< 6+(current_drive_music_count*2))
                        {
                            cord_changed=1;
                            Bk_color(117,7);
                            song_selector(a1,a2,b1,b2);
